...
for f do
    ...
    cut -f2 $T.nf+T+sym{0,1} |perl -pe chomp >$T.cdata
    cut -f2 $T.nf |frenc-mini >>$T.cdata
    cut -f3 <$T.nf+T+sym1 |uniq |frenc-mini >>$T.cdata

    ${0%/*}/packrec $T.sym0i $T.fsym $T.n0dup $T.cdata >$b.rec
done
